...
for i in range(10000):
    ...
    if i % 1000 == 0:
         print(f"Iteration {i}, Loss: {loss_value.asnumpy()}")
...
def forward_fn_no_tsa(x, y):
    logits = model(x)
    loss_value = loss_fn(logits, y)
    return loss_value

grad_fn_no_tsa = mindspore.ops.value_and_grad(forward_fn_no_tsa, grad_position=None, weights=model.trainable_params())

for i in range(10000):
    loss_value, grads = grad_fn_no_tsa(inputs, targets)
    optimizer(grads)
